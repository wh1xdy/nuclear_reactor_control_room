...
def main():
    ...
    while running:
        for ev in pygame.event.get():
            if ev.type == pygame.QUIT:
                ...
                if ev.key == pygame.K_ESCAPE:
                    ...
                    supervisor.shelve_current_alarms(now=time.time(), seconds=20.0)
        ...
        status_lines = [
            ...
            f"Unacked alarms: {len(snapshot.unacked_alarms)} (C=ack, U=shelve)",
            f"Diagnostics: {len(snapshot.diagnostics)} active",
        ]
        ...
        # diagnostics
        y_diag = 440
        screen.blit(font.render("Diagnostics", True, (170, 210, 255)), (790, y_diag))
        y_diag += 22
        if snapshot.diagnostics:
            for d in snapshot.diagnostics[:3]:
                screen.blit(font.render(f"- {d}", True, (255, 170, 120)), (790, y_diag))
                y_diag += 18
        else:
            screen.blit(font.render("- none", True, (160, 180, 200)), (790, y_diag))
        ...
        y_start = 500
        screen.blit(font.render("Startup permissive checklist", True, (180, 220, 180)), (220, y_start))
        y_start += 24
        for k, ok in snapshot.startup_checklist.items():
            mark = "[x]" if ok else "[ ]"
            col = (120, 220, 140) if ok else (220, 120, 120)
            screen.blit(font.render(f"{mark} {k}", True, col), (220, y_start))
            y_start += 20

        help1 = font.render(
            "W/S rods A/D flow Q/E turbine F/V feedwater H/N pressurizer C=ack U=shelve L=reset B/G bypass I inhibit SHIFT=fast R=reset ESC=quit",
            True,
            (160, 170, 185),
        )
